...
**Current version: 1.8.51** — see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
| **Village templates** | JSON templates under `templates/`; progress in `templates/progress.json`. Experimental/standalone templates (e.g. `village_stage_fast_basic_15c` for crop-heavy villages) aren't in the default chain — assign one to a village from the terminal menu (**B → pick village → pick template**). A step blocked purely on Warehouse/Granary *capacity* auto-relieves itself by upgrading storage out of order rather than retrying forever; any village stuck on the same blocked status 4+ times in a row logs a `repeated_blocked` warning. |
