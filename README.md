...
**Current version: 1.8.104** — see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
- **Login times out waiting for `input[placeholder="Enter your username"]`, failure URL is `https://nexian.world/?journey=true` (v1.8.104):** check the startup log for a line reading `Using NEXIAN_URL override: https://nexian.world/ (...)`. If you see it, you have a stray **active** `NEXIAN_URL=https://nexian.world/` line left over in `.env` from before v1.8.103 (it was silently added by a bug fixed in v1.8.101, and — unlike `NEXIAN_USERNAME`/`PASSWORD`/`GAME_HOST` — is intentionally never auto-removed since v1.8.103, as it's still a valid opt-in override for anyone who wants it). It permanently overrides `GAME_HOST`, so the bot opens the bare `https://nexian.world/` portal home page instead of the realm-specific smart URL that auto-opens the login modal — leaving it to fall back to clicking "Play Now" + a realm card, which can time out if the portal's front-end markup doesn't match what that fallback expects. Delete or comment out that `NEXIAN_URL` line in `.env` and rerun. v1.8.104 added the log line above specifically so this no longer requires opening `.env` blind to check.
