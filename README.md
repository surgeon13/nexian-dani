...
**Current version: 1.8.115** — see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
- **Brand-new villages get permanently stuck on `[Builder Loop] Iron Foundry is locked until Main Building reaches level 5 (currently 0). No template step manages Main Building...`, repeating "Switching villages (RR) due to temporary block" forever and never reaching Main Building level 5 or later village-stage buildings:** fixed in **v1.8.114** — a real bug in `BUILDER_RR_INTERLEAVE_RESOURCE_VILLAGE`'s turn-alternation counter (on by default since v1.8.113). It was supposed to advance once per real turn taken, but was actually being advanced every time `villageHasPendingBuilderWork()` merely *checked* a village — which happens for every non-excluded village, multiple times per tick, just from RR candidate-list filtering, regardless of whether that village was actually picked. That decoupled the alternation from real completed builds entirely, so a brand-new village's resource plan (which needs Main Building ≥ 5 for its Iron Foundry bonus building, but has no template step of its own that can build Main Building — only the village plan does) could keep landing on "resource" turn after turn by coincidence, hitting the same immediate block indefinitely while "village" mode — the only thing that could actually fix it — never got a real turn. Upgrade to pick this up automatically; no setting to change. If a village is already stuck on this exact message, it should self-resolve within a few of its next real RR turns once alternation is working correctly again.
