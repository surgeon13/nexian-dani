...
**Current version: 1.8.125** — see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
- **Builder:** `BUILDER_GOLD_COMPLETE_*`, `BUILDER_MASTER_BUILDER_ENABLED`, `BUILDER_ROUND_ROBIN_ENABLED`, `BUILDER_DEFAULT_PLAN_MODE` (`resource` or `village`), `BUILDER_RR_RESOURCE_THEN_VILLAGE` (continue into village-stage once resource fields finish), `BUILDER_RR_INTERLEAVE_RESOURCE_VILLAGE` (alternate resource/village turns instead of finishing resource fields entirely first — **on by default** as of v1.8.113)
...
- **Login times out waiting for `input[placeholder="Enter your username"]`, failure URL is `https://nexian.world/?journey=true`:** as of **v1.8.105** this fixes itself automatically — no `.env` edit needed. This is caused by a stray **active** `NEXIAN_URL=https://nexian.world/` line left over in `.env` from before v1.8.103 (silently added by a bug fixed in v1.8.101; unlike `NEXIAN_USERNAME`/`PASSWORD`/`GAME_HOST`, `NEXIAN_URL` is intentionally never auto-removed, since it's still a valid opt-in override for anyone who wants it). It permanently overrode `GAME_HOST`, sending the bot to the bare `https://nexian.world/` portal home page instead of the realm-specific smart URL that auto-opens the login modal — leaving it to fall back to clicking "Play Now" + a realm card, which timed out. v1.8.104 first made this visible with a startup log line; **v1.8.105 goes further and ignores this exact leftover value automatically**, falling back to the smart `GAME_HOST`-based URL on its own (logged as `Ignoring NEXIAN_URL=... — this is the exact bare-portal value a since-fixed auto-repair bug used to silently inject...`). A genuinely custom `NEXIAN_URL` (anything other than the bare `https://nexian.world/` / `https://nexian.world`) is untouched and still works exactly as before.
...
- **Village-stage buildings (Warehouse, Granary, Main Building, ...) never build, only resource fields do:** fixed in **v1.8.96** — `BUILDER_RR_AUTO_EXCLUDE_ON_RESOURCE_COMPLETE` and `BUILDER_RR_RESOURCE_THEN_VILLAGE` (both default `true`) were checked in the wrong order, so a village's resource-fields chain finishing immediately excluded it from Round Robin before village-stage ever got a turn. Upgrade, then check terminal menu → Settings → **[X] Builder RR Exclusion** — any village already excluded with reason "Resource fields complete" from before the fix needs to be manually removed from that list to resume. As of **v1.8.117**, that same menu has an **`[E]` Exclude all** action (alongside the existing `[A]` Clear all exclusions) to pause Builder RR for every village at once — e.g. before a manual pass, then `[A]` to resume all of them together.
...
- **Troop Auto looks stuck between branches (Builder Loop / dashboard commands also "Still waiting" with no resolution):** locating a branch's building can fall back to probing up to 22 inner slots if it's not on the village map, in the configured URL, or already cached — as of **v1.8.92** this is capped to a bounded budget (**v1.8.94**: 20s, was 90s in v1.8.92, previously unbounded) and logs `probing inner building slots...` so it's visible instead of silent. Upgrade if you still see this. If the same branch keeps hitting this every cycle even though its building exists in-game, that's a detection bug for that building type worth reporting (with the village map markup for it) rather than something the budget alone fixes.
...
- **Pressing `[2]` Village Stage Builder only ever ran resource-field steps (Warehouse/Granary/Main Building never got touched), even though `[3]` Resource Fields Builder worked fine:** fixed in **v1.8.109**. A 1.8.53 fix (preventing a specific conflict — pressing the *other* key on a village pinned to a standalone template) went further than intended and made manual `[2]`/`[3]` defer *entirely* to the same "resource before village" sequencing the auto Round Robin loop uses. Under the shipped default combo (`BUILDER_DEFAULT_PLAN_MODE=resource` + `BUILDER_RR_RESOURCE_THEN_VILLAGE=true`), that meant pressing `[2]` silently ran a resource-fields step instead for any village whose resource plan wasn't 100% finished yet — normal for most villages most of the time, so `[2]` looked broken. The manual keys now honor whichever one you actually pressed for that one step, still deferring only for the one genuine conflict case (a village pinned to a standalone template outside either default chain) — if that's why yours differs, the log line now says so explicitly instead of substituting silently.
- **`[Builder Loop] No non-capital villages available for template auto-build. Skipping.` — nothing ever builds, even with `BUILDER_LOOP_ENABLED`/`BUILDER_ROUND_ROBIN_ENABLED` both on:** fixed in **v1.8.110** — Builder Round Robin deliberately excludes the capital (it's meant for round-robining newer off-villages while the capital is built manually or via the non-RR loop), which left any **single-village account** (no expansion yet — the normal state right after a fresh server start) with zero RR candidates, forever, logging exactly this warning every tick. As of v1.8.110, RR falls back to building the capital directly when it's genuinely the *only* village; any account with at least one real off-village keeps the original capital-excluded behavior unchanged. If you still see this exact warning after upgrading, it means the village list itself isn't reading correctly (zero villages detected at all, not just zero non-capital ones) — that's a different, real bug worth reporting with what `V` (village selector) shows.
- **Village infrastructure (Warehouse, Granary, Main Building, ...) only gets built *after* every resource field is already maxed, instead of alongside them:** this was the default `BUILDER_RR_RESOURCE_THEN_VILLAGE` behavior through v1.8.110 — resource fields finished entirely (all 18 basic fields + bonus buildings) before a single village-stage step ever ran, which could be a long stretch with no capacity/build-speed support from those buildings the whole way there. `BUILDER_RR_INTERLEAVE_RESOURCE_VILLAGE` (introduced opt-in in v1.8.111, **on by default as of v1.8.113** per explicit request — "we want to be able to let it work and refine villages automatically with the templates") alternates turns instead — one resource-field step, then one village-stage step, back and forth — so both templates progress together per village rather than one strictly gating the other. Set it to `false` in `templates/settings.json` to restore the original strict sequencing. Requires `BUILDER_ROUND_ROBIN_ENABLED` + `BUILDER_RR_RESOURCE_THEN_VILLAGE` both on (the interleave only kicks in within that combined pipeline, both on by convention) — manual `[2]`/`[3]` keys are unaffected either way, they already run whichever mode you press. **Upgrading from before v1.8.113:** if `templates/settings.json` already exists with this key explicitly set to `false`, the new default doesn't override it — edit the file (or delete the key and let it re-default) to actually pick up the new default.
- **Changed a template's step order (e.g. `templates/village_stage_00.json`) but an already-in-progress village seems to skip a step or build things in the wrong order:** expected, and not a bug — a village's position in a template is stored as a numeric `stage_index`/`step_index` in `templates/progress.json`, and that index isn't re-derived from live building levels when the template's *content* changes, only when it's out of range entirely. A brand-new village with no prior progress always starts correctly at the template's new first step; a village that was already partway through the **old** version of a template needs its progress re-pointed at the new one. Terminal menu **`[B]` Builder Templates** → re-assign that village to `village_stage_00` (or whichever template changed) resets its `stage_index`/`step_index` to 0, so it picks up the new order cleanly from the start — safe even mid-progress, since every step is `build_or_upgrade` to a target level (already-met targets are a no-op, nothing gets rebuilt or downgraded).
- **`[Builder Loop] Iron Foundry/Sawmill/Brickyard is locked until Main Building reaches level 5 (currently 0). No template step manages Main Building...`, then "Switching villages (RR) due to temporary block" — one village after another all hitting this identical wall, never building Main Building:** v1.8.114 fixed a real bug in the interleave alternation counter that made this worse than intended, but v1.8.115 evidence showed the deeper issue: every RR-managed village's very *first* resource-mode turn can hit this exact block (a resource-plan bonus building needs Main Building level 5, but only the village plan can build Main Building) — and switching to a different village each time meant it could take many RR cycles before any specific village got a "village mode" turn to actually fix it, especially with several new villages in rotation all hitting the identical wall. **Fixed properly in v1.8.116**: hitting this specific block in resource mode now switches to village mode for the *same* village immediately, same tick, instead of moving to a different village — Main Building gets built right then, not whenever RR eventually cycles back around. Requires `BUILDER_ROUND_ROBIN_ENABLED` + `BUILDER_RR_RESOURCE_THEN_VILLAGE` both on (same combined-pipeline requirement as interleaving); upgrade to pick this up automatically, no setting to change.
- **A stream of `[Builder Loop] progress_advanced: ... already at level N (target: M). Advancing. Retrying next step...` messages, one after another, then the RR loop moves on to a different village with nothing actually built:** fixed in **v1.8.115** — every tick's follow-up loop (the thing that walks through already-satisfied steps to catch a village's tracked progress up to where its fields/buildings actually already are — common after `BUILDER_SPEED_BUILD_ENABLED`'s one-click upgrades race ahead of `templates/progress.json`, or after a template edit) was capped at 20 catch-up steps / 2 minutes per tick, shared with genuine build actions. A village needing to catch up through more than ~20 already-satisfied steps at once (an entire resource template's 18 field slots, sometimes stacked across more than one stale template in the chain) hit that cap purely doing housekeeping, before ever reaching a real, buildable step — so the tick ended with nothing built, over and over. Raised to 200 steps / 4 minutes. Safe by construction: every status this budget is spent on is a no-op (no real click happens); the loop always stops immediately the moment it reaches a genuine build action, so this change can only let it catch up further, never cause extra real actions per tick.
- **Terminal spammed with a long run of `[Builder Manual] already_satisfied: ... Retrying next step...` (or `[Builder Loop]` equivalent) for every already-maxed field/building, with nothing real happening in between:** the raised catch-up budget above (v1.8.115) fixed the underlying "nothing gets built" bug, but as of **v1.8.118** the per-step logging for that same catch-up walk is muted — `already_satisfied`, `skipped_wrong_building_type`, `skipped_village_full`, and `template_complete` are pure background housekeeping (the tracker is fast-forwarding past a step the live game already meets; no click happens), so they're counted instead of printed one-by-one, and summarized in a single `[Builder Loop] Caught up N already-satisfied step(s) for <village> (no build needed).` line once the walk finishes (only printed if N > 0). `realigned_template`, `storage_relief`, and `prerequisite_relief` still log every step, since each of those represents a real, alternate build click actually landing — and any genuine `success` build step already logs on its own as before. No setting to change; this only affects what prints, not what builds.
- **`[Builder Loop] repeated_blocked (N): ... in village plan keeps hitting 'blocked_mismatch' ... Slot 30 contains 'Sawmill', expected 'Town Hall'. Will not click upgrade on a mismatched building.` — stuck for many consecutive ticks, Town Hall never built even though the village has one:** fixed in **v1.8.119** — village-stage templates hardcode slot 30 for Town Hall, but unlike Main Building/Warehouse/Granary/Marketplace (built in a fixed, earliest order), Town Hall is built later, by which point an earlier flexible bonus building (Sawmill, Brickyard, Iron Foundry, Grain Mill, Bakery — all of which already needed live-map slot discovery instead of a guessed template slot, same as Residence/Palace) can already occupy slot 30. The game itself reaches Town Hall by building type (`build.php?gid=24`), not by a fixed slot — `celebrations.js` already does exactly that — but `villageBuilder.js`'s builder-step slot check still trusted the template's guessed slot 30 outright. Town Hall now gets the same live-map discovery treatment: if the guessed slot doesn't hold it, the bot searches the rest of the village for wherever it actually is before giving up. Upgrade to pick this up automatically — no setting to change, and this only widens discovery for Town Hall specifically (Main Building/Warehouse/Granary/Marketplace/Rally Point/Barracks/Academy/Smithy/Stable remain on their fixed template slots, unaffected).
  - **Still seeing the identical `Slot 30 contains 'Sawmill', expected 'Town Hall'` message after upgrading to v1.8.119?** That fix relies on live-map discovery kicking in at runtime — it's applied to the code, but a **long-running bot process started before you pulled/restarted doesn't pick it up until you restart it** (`npm run cursor:ensure` / `npm run start:24-7`, or your PC equivalent). Separately, **v1.8.120** also corrects `templates/village_stage_01.json`/`village_stage_02.json`'s Town Hall step itself from the guessed slot 30 to **slot 23** — the actual, repeatedly-confirmed slot across multiple real villages on the same account (Sawmill really is the one that belongs on 30). This makes the very first read match immediately instead of needing a discovery round-trip, and removes any dependency on that discovery path working perfectly on your server. If your account's own layout differs, the flexible discovery from v1.8.119 is still there as a fallback.
- **Templates repeatedly get stuck partway through, resource bonus buildings never finish and inner buildings never keep getting pushed, one or more villages seemingly stall forever — often preceded by `[Builder Loop] repeated_blocked (N): ... keeps hitting 'blocked_master_builder_only' ... Slot X has only a Master Builder upgrade button. Enable Master Builder in settings or upgrade manually.`:** fixed in **v1.8.121** — `blocked_master_builder_only` (the village's one free build queue slot is currently occupied by something else already in progress — completely normal for any account without Gold Club's second concurrent build slot) was misclassified alongside genuinely-stuck statuses like `blocked_mismatch`, so a village that was simply *busy building* could rack up enough consecutive hits to trip `BUILDER_RR_AUTO_EXCLUDE_BLOCKED_STREAK` (default 12) and get **silently, permanently excluded from Builder RR** — stopping both its resource-field/bonus-building progress and its village-stage/inner-building progress at once, over a condition that was never actually stuck. It now joins `blocked_resources`/`blocked_queue`/`blocked_storage`/`idle_saturated` as transient: it still waits out its normal 5-minute cooldown before retrying that village, but can never trigger auto-exclusion on its own. If a village was already excluded for this reason before upgrading, check terminal menu → Settings → **`[X]` Builder RR Exclusion** and remove it manually (excludes recorded before this fix aren't reverted automatically) — the exclusion reason is logged, so `Stuck on 'blocked_master_builder_only' for N consecutive ticks` in your history identifies exactly which ones. A live 24/7 process needs a restart to pick this fix up, same as the Town Hall fixes above.
- **`resource_fields_03`/`resource_fields_04` never advances past placing Sawmill/Brickyard/Iron Foundry — fields stay capped around level 8-10, `[Builder Loop] Iron Foundry is locked until Main Building reaches level 5 (currently 0). No template step manages Main Building...`, switches to village mode, but the village's own village-stage progress shows it's already far past Main Building's construction stages:** fixed in **v1.8.122** — `resource_fields_03` is `sequence_mode: "strict"`, so if placing a bonus building (which needs Main Building ≥ 5) gets blocked, nothing after it in that template — or in `resource_fields_04`, which is where the *other* 14 fields finally reach level 10 — can ever run. The block itself was a false alarm: `readPrerequisiteBuildingLevel()` (which checks Main Building's live level for this exact prerequisite) relied solely on the village-map survey to find it, and that survey's markup isn't reliable on every server/tribe — same root weakness the Town Hall fix (v1.8.119) already worked around for a different building. When the survey missed Main Building, its level was silently reported as `0` even when it was plainly built to a much higher level, permanently blocking the bonus-building placement step no matter how long you waited. `discoverInnerBuildingSlotFromMap()` (used for Main Building, Barracks, and any other non-flexible inner building this prerequisite check needs) now falls back to reading every inner slot directly when the survey comes up empty — the same fallback flexible bonus buildings already had — and caches the result so this doesn't cost a fresh ~22-page probe on every tick. Upgrade and restart to pick this up automatically — no setting to change.
- **`[Builder Loop] realigned_template: Target bonus building 'Sawmill'/'Brickyard'/'Iron Foundry' is locked on slot N. Realigning to Stage 1 prerequisite resource fields to continue progressing. Retrying next step...` — the identical message repeating every ~20s forever, no progress:** fixed in **v1.8.123** — whenever placing a bonus building was blocked for any reason *other* than the Main Building-level check (e.g. still locked even though resources and Main Building's level both check out), `resource_fields_03` unconditionally reset its progress back to Stage 1 and retried. Since Stage 1's fields are already satisfied once the chain has run a while, that reset walked straight back to the exact same blocked step and reset again — forever, with the follow-up loop burning its whole retry budget on a no-op every single tick. Placing a bonus building now only gets **one** automatic Stage-1 realign attempt; if it's still blocked the next time around, it falls through to the same `blocked_target_locked`/`blocked_target_unavailable` statuses every other template already gets for this situation, so the normal blocked-streak warning and (after enough consecutive ticks) Builder RR auto-exclusion can actually respond instead of spinning silently. If a village hits this, it means something needs checking in-game (why is the slot still locked — culture points, a different unmet requirement, etc.) rather than something the bot can resolve on its own by retrying. Upgrade and restart to pick this up — no setting to change.
  - **v1.8.123's fix immediately surfaced what it had been masking: on real accounts, that "something needs checking in-game" was almost always "there's no room left."** After upgrading, every village in Builder RR reported `[Builder Loop] repeated_blocked (N): ... 'blocked_target_locked' ... Target building 'Iron Foundry' is listed but not currently buildable for slot 25. Buildable now: Cranny, Embassy, Great Barracks, Great Stable, Hero's Mansion, Treasury.` and `Switching villages (RR) due to temporary block` on every single one, so nothing built anywhere — reported live as *"now the builder is all broken, nothing is built at all only repeating loops."* Fixed in **v1.8.124**: mature villages that already have most of their inner buildings (Treasury, Great Barracks, Great Stable, Hero's Mansion, etc., often built manually or via other means) can genuinely have zero free inner sites left for Sawmill/Brickyard/Iron Foundry — the game still *lists* those as buildable types, just greyed out, which is indistinguishable from a real temporary lock unless you check whether any empty site remains anywhere in the village. That check (`isNewBuildingUnplaceable()`) already existed and was already used when a building wasn't listed as an option *at all* — it just wasn't consulted for the "listed but currently locked" case these villages were hitting. It's now checked there too: a genuinely full village skips that one bonus-building step (`skipped_village_full`) and moves on with the rest of its resource-field progress, instead of blocking (or endlessly realigning) forever over a building that structurally has nowhere to go. Upgrade and restart to pick this up — no setting to change.
  - **v1.8.124's village-full check still wasn't the whole story: `blocked_target_locked`/`blocked_prerequisite_building` kept repeating on `Iron Foundry` even in villages with Main Building at level 20 — far above the assumed level-5 requirement.** Confirmed live: the user checked in-game and found Main Building genuinely at level 20 in the affected village, and that the resource-field slot the template assumed was an Iron Mine (slot 9, per the standard 1-4=Woodcutter/5-8=Clay Pit/9-12=Iron Mine/13-18=Cropland layout every template guesses) was actually a **Cropland**. Fixed in **v1.8.125**: `resource_fields_03`'s "one of each resource to 10" prerequisite steps (Stage 1 — the ones that specifically exist to guarantee a genuine field of each type reaches level 10, since that's what the game itself checks before allowing Sawmill/Brickyard/Iron Foundry/Grain Mill) were using the same "any resource field type satisfies this step" generic matching every other resource template step uses for convenience. That's fine when the end goal is "upgrade all 18 fields eventually" — it's wrong here, because it let a high-level Cropland silently satisfy an "Iron Mine → 10" requirement, so the tracker believed the prerequisite was met while no genuine Iron Mine anywhere in the village ever reached level 10 — permanently blocking Iron Foundry no matter how high Main Building went. Those 4 steps now use `strict_match: true` (disabling the generic substitution) plus a new live discovery step: when the guessed slot doesn't hold the genuine type, the bot searches the rest of the village's 18 field slots for the real one (picking the highest-level match) and targets that instead — the same "don't trust a guessed slot, verify/discover the real one" pattern already applied to Town Hall (v1.8.119) and Main Building (v1.8.122). If no genuine field of that type exists anywhere at all, it now reports an honest `blocked_mismatch` instead of silently faking success. Upgrade and restart to pick this up — no setting to change.
