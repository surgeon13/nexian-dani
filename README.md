...
**Current version: 1.8.112** — see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
- **Builder:** `BUILDER_GOLD_COMPLETE_*`, `BUILDER_MASTER_BUILDER_ENABLED`, `BUILDER_ROUND_ROBIN_ENABLED`, `BUILDER_DEFAULT_PLAN_MODE` (`resource` or `village`), `BUILDER_RR_RESOURCE_THEN_VILLAGE` (continue into village-stage once resource fields finish), `BUILDER_RR_INTERLEAVE_RESOURCE_VILLAGE` (alternate resource/village turns instead of finishing resource fields entirely first — off by default)
...
- **Village infrastructure (Warehouse, Granary, Main Building, ...) only gets built *after* every resource field is already maxed, instead of alongside them:** this is the default `BUILDER_RR_RESOURCE_THEN_VILLAGE` behavior — resource fields finish entirely (all 18 basic fields + bonus buildings) before a single village-stage step ever runs, which can be a long stretch with no capacity/build-speed support from those buildings the whole way there. As of **v1.8.111**, set `BUILDER_RR_INTERLEAVE_RESOURCE_VILLAGE: true` in `templates/settings.json` to alternate turns instead — one resource-field step, then one village-stage step, back and forth — so both templates progress together per village rather than one strictly gating the other. Off by default; existing installs are unaffected unless you turn it on. Requires `BUILDER_ROUND_ROBIN_ENABLED` + `BUILDER_RR_RESOURCE_THEN_VILLAGE` both on (the interleave only kicks in within that combined pipeline) — manual `[2]`/`[3]` keys are unaffected either way, they already run whichever mode you press.
